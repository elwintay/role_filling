...
# def create_sent_tagging(doc_dict, keys_dict_none_empty):
    ...
    max_len = len(seqs_all_o) if len(seqs_all_o)<=len(seqs_not_all_o) else len(seqs_not_all_o)
    seqs_all_o_sample = random.sample(seqs_all_o, max_len)
...
if __name__=='__main__':
    ...
    p.add_argument('--div', default='train', choices=['train', 'dev', 'test', 'raw'])
    p.add_argument('--template_type', default='attack', choices=['attack', 'kidnapping', 'bombing','others'])
    args = p.parse_args()
    ...
    if args.template_type=='attack':
        read_path = "../../data/processed_attack/"
        save_path = "./data_seq_tag_pairs_attack/"
    elif args.template_type=='kidnapping':
        read_path = "../../data/processed_kidnapping/"
        save_path = "./data_seq_tag_pairs_kidnapping/"
    elif args.template_type=='bombing':
        read_path = "../../data/processed_bombing/"
        save_path = "./data_seq_tag_pairs_bombing/"
    else:
        read_path = "../../data/processed/"
        save_path = "./data_seq_tag_pairs/"
    # doc_dict, keys_dict = read_files(doc_file, keys_file)
    if (args.div == 'test') or (args.div == 'raw'):
        with open(read_path + args.div + ".json", "r") as doc_keys_file:
            doc_keys = json.load(doc_keys_file, object_pairs_hook=OrderedDict)
    else:
        with open(read_path + args.div + "_full.json", "r") as doc_keys_file:
            doc_keys = json.load(doc_keys_file, object_pairs_hook=OrderedDict)
    ...
    if (args.div == 'test') or (args.div == 'raw'):
        ...
        f = open(save_path + args.div, "w+") 
    else:
        output_examples = all_examples_sample_neg
        f = open(save_path + args.div + "_full", "w+") 
